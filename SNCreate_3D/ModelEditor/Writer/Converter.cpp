...
void Converter::ReadSkinData()
{
	for (UINT i = 0; i < scene->mNumMeshes; i++)
	{
		...
		if (aiMesh->HasBones() == false)continue; //본이없으면 패스
		...
		for (UINT b = 0; b < aiMesh->mNumBones; b++) //매시에 연결된 본들
		{
			...
			{
				UINT index = aiMeshBone->mWeights[w].mVertexId; //영향을 받는 정점들
...
void Converter::ReadMaterialData()
{
	for (UINT i = 0; i < scene->mNumMaterials; i++)
	{
		...
		srcMaterial->Get(AI_MATKEY_COLOR_AMBIENT, color);//구글에 그대로 Assimp관련으로 검색
		material->Ambient = Color(color.r, color.g, color.b, 1.0f);//불투명도는 빛의 강도로 표현되기도함.
